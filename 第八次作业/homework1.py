...
fig = plot_acf(data['中国:固定资产投资完成额:累计值'], lags=50, ax=ax1)
ax2 = fig.add_subplot(212)
fig = plot_pacf(data['中国:固定资产投资完成额:累计值'],lags=50, ax=ax2)
# plt.show()
...
for p in range(pmax+1):
    ...
    for q in range(qmax+1):#存在部分报错，所以用try来跳过报错。
        try:
            tmp.append(ARIMA(data['中国:固定资产投资完成额:累计值'],order=(p,0,q)).fit().bic)
        except:
            tmp.append(None)
...
armodel200 = ARIMA(tuple(data['中国:固定资产投资完成额:累计值']),order=(3,0,3)).fit()
resid = armodel200.resid
# print(dir(resid))
